...
block_cov <- function(freemats, fr, mat, skel, Ng, dosam = FALSE) {
  ...
  if (nrow(blkse) > 0) {
    if (dosam) {
      ...
      blkse <- blkse[(blkse[,3] == 1) & (blkse[,2] - blkse[,1] > 0), , drop = FALSE]
    }
  ...
  list(out = out, frnoblock = frnoblock, blkmats = blkmats)
}
